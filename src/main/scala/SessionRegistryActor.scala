...
class SessionRegistryActor(props: Props) extends Actor with ActorLogging {
  ...
  def receive(actors: Map[String, ActorRef],
              clientsConnections: Map[String, ActorRef]): Receive = {
    ...
    case AskForSID(eio, sessionId) => //First connection
      val result: String = sessionId match {
        ...
        case None =>
          ...
          val fromActor
            : Sink[SessionRegistryActor.ActorRegistryCommunication, NotUsed] =
            Flow[SessionRegistryActor.ActorRegistryCommunication]
              .map {
                case SessionRegistryActor.Pong(key) =>
                  SessionRegistryActor.WrappedPong(sid, key)
                case SessionRegistryActor.Ping(key) =>
                  SessionRegistryActor.WrappedMessage(sid, key) // server should not send Ping
                case SessionRegistryActor.Message(text) =>
                  SessionRegistryActor.WrappedMessage(sid, text)
                case SessionRegistryActor.BroadcastOutgoingMessage(
                    text,
                    excludeMe
                    ) =>
                  SessionRegistryActor.WrappedBroadcastOutgoingMessage(
                    sid,
                    text,
                    excludeMe
                  )
              ...
              .to(
                Sink.actorRef[
                  SessionRegistryActor.ActorRegistryCommunicationWrappers
                ](self, Disconnect(sid))
              )

          val toActor = Source
            .actorRef[SessionRegistryActor.ActorRegistryCommunication](
              10,
    ...
    case UpdateOut(sid, out) => //Register Out for SID (where to send)
      ...
      actors.get(sid) match {
        ...
        case None =>
          log.error("UpdateOut cannot find out actor for sid - {}", sid)
      }
    ...
    case Disconnect(sid) =>
      val newIncoming = actors.get(sid) match {
        ...
        case None =>
          log.error(
            "Disconnect:newIncoming cannot find out actor for sid - {}",
            sid
          )
          actors
      }
      val newOutgoing = clientsConnections.get(sid) match {
        ...
        case None =>
          log.error(
            "Disconnect:newOutgoing cannot find out actor for sid - {}",
            sid
          )
          clientsConnections
      }
    ...
    case IncomingMessage(sid, packetPattern("4", pDataId, pData)) =>
      actors.get(sid) match {
        case Some(actor) =>
          actor ! Message(pData)
        case None =>
          log.error("MESSAGE cannot find out actor for sid ({})", sid)
      }
    ...
    case IncomingMessage(sid, message) =>
      log.warning("Unknown IncomingMessage ({} - {})", sid, message)
    ...
    case WrappedPong(sid, pData) =>
      clientsConnections.get(sid) match {
        ...
        case None =>
          log.warning("WrappedPong - cannot find actor for sid - {}", sid)
      }
    ...
    case WrappedBroadcastOutgoingMessage(sid, text, excludeMe) =>
      clientsConnections foreach {
        case (clientId, out) =>
          if (!excludeMe || clientId != sid) out ! OutgoingMessage(s"42$text")
      }
...
object SessionRegistryActor {
  ...
  private case class WrappedPing(sid: String, key: String)
      extends ActorRegistryCommunicationWrappers

  private case class WrappedPong(sid: String, key: String)
      extends ActorRegistryCommunicationWrappers

  private case class WrappedMessage(sid: String, text: String)
      extends ActorRegistryCommunicationWrappers

  private case class WrappedBroadcastOutgoingMessage(sid: String,
                                                     text: String,
